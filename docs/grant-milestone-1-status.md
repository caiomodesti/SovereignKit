...
The current runtime MUST NOT be described as a completed independent observation layer. The software and deployment path are implemented and locally integrated, so the status is `IMPLEMENTED_NOT_VALIDATED`. It remains below `ACCEPTED` until three real external deployments, real reader calls, failure/recovery exercises, and independence evidence are retained.
