...
- a fail-closed 24-hour Collector canary retained 1,441/1,441 ready samples with 100% coverage and zero storage regressions on the corrected E4 host;
- the E4 public edge now has retained DNS correlation and publicly trusted TLS evidence while exposing only schema-validating `POST /v0/probe-results`; `/health` remains private.
...
7. TLS terminates at a Caddy edge and the initial DNS, certificate, NSG, guest-firewall, and route-boundary gates passed. Rate limiting, automated certificate-expiry alerting, ongoing disk monitoring, retention, backup, and successful signed remote Observer delivery remain absent.
