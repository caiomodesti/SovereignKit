...
$ledger = Join-Path $env:TEMP ('SovereignKit\sprint-3-router-' + $stamp)
$runDirectory = Join-Path $workspace ('artifacts\sprint-3\runs\' + $stamp)
...
Set-Content -LiteralPath (Join-Path $validatorDirectory 'validator.pid') -Value $validatorProcess.Id -Encoding ascii
Set-Content -LiteralPath (Join-Path $validatorDirectory 'ledger-path.txt') -Value $ledger -Encoding utf8
...
try {
  ...
  if ($UpdateFixture) {
    $env:SOVEREIGNKIT_FIXTURE_DIR = Join-Path $workspace 'fixtures\integration\agave-4.0.0\router-failover'
  } else {
    Remove-Item Env:SOVEREIGNKIT_FIXTURE_DIR -ErrorAction SilentlyContinue
  }
  & node node_modules/typescript/bin/tsc -p packages/sdk/tsconfig.integration.json --noEmit
  if ($LASTEXITCODE -ne 0) { $testExitCode = $LASTEXITCODE; throw 'Integration typecheck failed' }
  & node node_modules/vitest/vitest.mjs run packages/sdk/integration/live-router.integration.test.ts --testTimeout=120000
  $testExitCode = $LASTEXITCODE
} finally {
  if (-not $validatorProcess.HasExited) {
    Stop-Process -Id $validatorProcess.Id
    Wait-Process -Id $validatorProcess.Id -Timeout 20 -ErrorAction SilentlyContinue
  }
  ...
  $manifest | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $runDirectory 'run-manifest.json') -Encoding utf8
