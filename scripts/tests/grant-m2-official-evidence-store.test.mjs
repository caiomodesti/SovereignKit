...
const collectorRecord = { collector_sequence: 2, collected_at: "2026-09-14T04:10:01.000Z", result: signed };
...
test("selects exactly one durable Collector record bound by result ID", () => {
  const other = { collector_sequence: 1, result: { result_id: "other" } };
  const log = `${JSON.stringify(other)}\n${JSON.stringify(collectorRecord)}\n`;
  assert.equal(selectGrantM2OfficialCollectorRecord(log, "result-1"), `${JSON.stringify(collectorRecord)}\n`);
  assert.throws(() => selectGrantM2OfficialCollectorRecord(`${log}${JSON.stringify(collectorRecord)}\n`, "result-1"), /ambiguous/u);
});
...
test("persists a complete immutable evidence set and permits exact crash recovery", async () => {
  ...
  assert.equal(Object.keys(stored.files).length, 7);
