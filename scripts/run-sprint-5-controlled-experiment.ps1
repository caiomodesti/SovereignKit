param(
  [string]$ValidatorPath = ".tools\agave\4.0.0-snapshotless\bin\solana-test-validator.exe",
...
$runDirectory = Join-Path $workspace ("artifacts\sprint-5\experiment-runs\" + $stamp)
$ledger = Join-Path $env:TEMP ("SovereignKit\sprint-5-experiment-" + $stamp)
$validatorDirectory = Join-Path $runDirectory "validator"
$fixtureDirectory = Join-Path $workspace "fixtures\integration\agave-4.0.0\controlled-experiment"
...
if ($healthy) {
  ...
  try {
    node node_modules/typescript/bin/tsc -p packages/analysis/tsconfig.integration.json --noEmit
    $testExitCode = $LASTEXITCODE
    if ($testExitCode -eq 0) {
      node node_modules/vitest/vitest.mjs run packages/analysis/integration/live-controlled-experiment.integration.test.ts --testTimeout=600000
      $testExitCode = $LASTEXITCODE
    }
  } finally {
    Pop-Location
