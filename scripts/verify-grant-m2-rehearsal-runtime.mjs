...
const expectedUnits = [
  'deploy/systemd/sovereignkit-m2-live-monitor.service',
  'deploy/systemd/sovereignkit-m2-live-monitor.timer',
  'deploy/systemd/sovereignkit-m2-observation-worker@.service',
  'deploy/systemd/sovereignkit-m2-official-observation-worker@.service',
];
if ((!hostPreparation && units.length > 0) || (hostPreparation && JSON.stringify(units) !== JSON.stringify(expectedUnits)) ||
    ...
    hostPreparation !== actualPaths.includes('scripts/install-grant-m2-official-host-controls.sh') ||
