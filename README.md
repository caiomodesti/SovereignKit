...
> **Status:** v0.1, Sprint 9 complete after the formal **GO** checkpoint. The controlled stack now includes a standalone Observer, durable local Collector, deterministic versioned intelligence snapshots, a local evidence dashboard, and fail-open probe-informed route ordering. No public hosted feed, Devnet validation, or decentralized observer network exists yet.
...
- 84/84 deterministic core, dashboard, intelligence, and route-order tests;
...
Sprint 8 provides a local static evidence console for routes, classes, observers,
controlled findings, failover, feed state, and methodology. See the [dashboard
design](docs/dashboard.md), [acceptance audit](docs/sprint-8-acceptance.md),
[hostile audit](docs/sprint-8-hostile-audit.md), and [reproduction
procedure](docs/sprint-8-reproduction.md).

Sprint 9 connects fresh, hysteresis-qualified intelligence to the router without
expanding or deleting locally eligible routes. See the [routing
contract](docs/probe-informed-routing.md), [acceptance
audit](docs/sprint-9-acceptance.md), [hostile
audit](docs/sprint-9-hostile-audit.md), and [reproduction
procedure](docs/sprint-9-reproduction.md).
...
- **SDK:** validates and polls versioned snapshots, applies TTL/hysteresis/override, rechecks freshness at routing time, and moves avoided routes behind locally preferred fallbacks without removing them.
- **Dashboard:** builds a static, read-only view from accepted experiment, observer, failover, and intelligence fixtures.
