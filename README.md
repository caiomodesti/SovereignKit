...
> **Status:** v0.1, Sprint 8 complete after the formal **GO** checkpoint. The controlled stack now includes a standalone Observer, durable local Collector, deterministic versioned intelligence snapshots, a fail-open polling client, and a local evidence dashboard generated only from accepted fixtures. No public hosted feed, probe-informed router integration, or decentralized observer network exists yet.
...
- 72/72 deterministic core and dashboard tests including standalone-process, loopback polling, evidence-state, and UI interaction paths;
